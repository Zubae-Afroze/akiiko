...
export const editShippingAddress = (index, address) => (dispatch, getState) => {
  try {
    let shippingAddress = getState().profile.userProfile.shippingAddress
    let updatedList = [...shippingAddress]
    console.log('shippingAddress: ' + updatedList)
    updatedList.splice(index, 1, address)
    ...
    axios.put(`/api/profile/editshipping/${uid}`, updatedList, config)
