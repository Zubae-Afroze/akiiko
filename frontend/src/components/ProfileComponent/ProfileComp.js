...
export default function ProfileComp({ profileDetails }) {
  ...
  const [phoneNumberFieldErrorState, setPhoneNumberFieldErrorState] =
    useState(false)
    ...
    if (editMobileFieldON) {
      element = (
        <input
          ...
          size='12'
          className={
            'input-filed-edit-style ' +
            (phoneNumberFieldErrorState ? 'edit-input-field-error' : '')
          }
          autofocus
  ...
  function handelNameUpdateEdit() {
    if (!editNameFieldON) {
      ...
      if (
        userProfileObject.firstName === null ||
        userProfileObject.firstName.trim() === ''
      ) {
        isvalid = false
        updatedList[0] = true
      }
      if (
        userProfileObject.lastName === null ||
        userProfileObject.lastName.trim() === ''
      ) {
        isvalid = false
        updatedList[1] = true
      }
      ...
      if (isvalid) {
        ...
        let name =
          userProfileObject.firstName + ' ' + userProfileObject.lastName
  ...
  function handelPhoneNumberUpdateEdit() {
    if (!editMobileFieldON) {
      ...
      if (
        userProfileObject.mobile === null ||
        userProfileObject.mobile.trim() === ''
      ) {
            ...
            <Row>
              ...
              <Col xs={8} md={5}>
                {editNameFieldON ? (
                  ...
                  <>
                    <span>{`${getName().firstName}`}</span> <span> </span>
                    <span>{`${getName().lastName}`}</span>
                  </>
                )}
              </Col>
              <Col xs={4}>
              ...
              <Col xs={4}>
                <h6
                  ...
                  onClick={() => handelPhoneNumberUpdateEdit()}
                >
  ...
  return (
    <>
      {profileDetails.shippingAddress.length === 0 ? (
        ...
        <AddressComp
          shippingAddress={profileDetails.shippingAddress[0]}
          index={0}
          key={0}
        />
      )}
